...
//
// Minimal for the auth phase (#7): session attach only. #9 folds in route
// protection + draft mode. Runs globally — session attach is cheap (cookie reads;
// a network call only in the refresh window), and downstream consumers (the
// /api/graphql proxy, page renders) need the refreshed session on every request.
